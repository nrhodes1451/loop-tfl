...
| OSM buildings (King’s Cross schematic) | Overpass `building=*` ways in a 400 m box around the TfL StopPoint; baked to `data/osm/hubkgx-buildings.json`. Optional London PMTiles extract (`npm run fetch-london-pmtiles`) is served from `/api/osm/london.pmtiles` and preferred when present |
...
npm run fetch-london-pmtiles  # optional Greater London PMTiles extract (gitignored)
npm run dev
```
...
- Schematic: every network station, invented layout, not to scale, not for wayfinding, isolated from routing. King’s Cross is hand-authored and overlays OSM buildings (400 m bake, or London PMTiles when `data/osm/london.pmtiles` is present).
